...
export default function NewSessionPage() {
  ...
  const { data: campaigns = [] } = useQuery<
    { id: number; include_history: boolean; notes: string | null }[]
  >({
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const data = await apiPost<{ publicId: string; inks: number }>(
  ...
  const saveOnlyMutation = useMutation({
    mutationFn: async () => {
      ...
      return { publicId: (data as { public_id: string }).public_id };
    },
    onSuccess: (data) => {
      clearDraft();
      queryClient.invalidateQueries({ queryKey: ["sessions", user?.id] });
  ...
  const handleSaveWithoutGenerating = (
  ...
  const generating = mutation.isPending || saveOnlyMutation.isPending;
  ...
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(ellipse at 50% 0%, #1e1830 0%, var(--color-ink) 60%)",
      }}
    >
      <AppHeader />
        ...
        <form onSubmit={handleGenerate} className="flex flex-col gap-6">
          {draftApplied && (
            ...
            >
              <span style={{ fontFamily: "var(--font-display)" }}>
                Draft restored
                {draft?.updated_at
                  ? ` · saved ${formatDraftAge(draft.updated_at)}`
                  : ""}
              </span>
              <button
                type="button"
            ...
            <div className="flex items-center gap-2">
              ...
              {historyEnabled && (
                <div className="relative group flex items-center">
                  ...
                  >
                    Previous session summaries from this campaign will be
                    included in the generation prompt to help the AI maintain
                    continuity.
                  </div>
                </div>
              )}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <p
              ...
              style={{ color: "var(--color-parchment-muted)" }}
            >
              Bullet points, prose, or a mix — any format works. Write in any
              language.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative group">
              ...
              >
                When enabled, the AI can add plausible dialogue, describe
                environments, and connect scenes with brief passages. It won't
                invent new characters or plot events — only colour what's
                already there.
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:items-start">
              <div className="flex-1 flex flex-col">
                <button
                  type="submit"
                  disabled={
                    generating || (profile !== null && profile.inks < 1)
                  }
                  className="flex-1 py-3 rounded-lg text-sm font-semibold tracking-widest uppercase transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                  style={{
                    fontFamily: "var(--font-display)",
                    background:
                ...
                <div className="flex items-center justify-center gap-1.5 mt-1 mb-2">
                  ...
                  >
                    {profile !== null && profile.inks < 1
                      ? "Not enough ink to generate"
                      : "Generate costs 1 ink"}
                  </span>
                </div>
              </div>
              <button
                type="button"
                onClick={handleSaveWithoutGenerating}
